...
import { AcmeAuthorization, AcmeChallenge } from "./types";
...
export interface AutoOptions {
  ...
  challengeCreateFn: ChallengeAction,
  challengeRemoveFn: ChallengeAction,
}
...
export interface ChallengeAction {
  (authz: AcmeAuthorization, challenge: AcmeChallenge, keyAuthorization: string): Promise<any>;
}
 ...
 *
 * @param {Client} client ACME client
 * @param {object} userOpts Options
 * @returns {Promise<buffer>} Certificate
 */

export async function auto(client, userOpts: AutoOptions) {
  const opts: AutoOptions = Object.assign({}, defaultOpts, userOpts);
  const accountPayload: AccountPayload = { termsOfServiceAgreed: opts.termsOfServiceAgreed };
