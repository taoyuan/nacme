...
async function verifyDnsChallenge(authz, challenge, keyAuthorization, prefix = "_acme-challenge.") {
  ...
  try {
    ...
    const cnameRecords: string[] = await PromiseA.fromCallback(cb => dns.resolveCname(challengeRecord, cb));
  ...
  /* Read TXT record */
  const result: string[][] = await PromiseA.fromCallback(cb => dns.resolveTxt(challengeRecord, cb));
  const records = (<string[]>[]).concat(...result);
